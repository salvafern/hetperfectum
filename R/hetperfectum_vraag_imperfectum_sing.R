...
#'
#' @examples
hetperfectum_vraag_imperfectum_sing <- function(){
  ...
  while(nrow(hetperfectum) > 0){
    ...
    aantwoord <- hetperfectum$Imperfectum_singularis[id]

    Sys.sleep(tijd)
    user_aantwoord <- readline(paste0('Wat is het singularis Imperfectum van "', vraag, '"?: '))
    Sys.sleep(tijd)
    ...
    if(user_aantwoord == aantwoord){
      ...
      message(paste0("Goed gedaan! Jij hebt ", juist, " juist imperfectum geschreven."))
    }else{
  ...
  Sys.sleep(tijd)
  message("Jij hebt alles imperfectum juist geschreven. Dankjewel voor spelen!! :)")
  Sys.sleep(tijd)
  return(hetperfectum_einde_spel(juist, fout))
}
